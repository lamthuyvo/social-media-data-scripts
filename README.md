...
```
3. Make a secrets.py file that is modeled after the `secrets.py.example` file by
```
...
#### Scripts
* [get_fb_posts_fb_group.py](#) or [get_fb_posts_fb_group_multiple.py](#): These scripts allow you to gather data from _public_ Facebook groups, either from just one or multiple groups. Adapted from [@minimaxir](https://github.com/minimaxir/facebook-page-post-scraper)'s scripts.
* [get_fb_posts_fb_page.py](#) or [get_fb_posts_fb_page_multiple.py](#): These scripts allow you to gather data from _public_ Facebook pages, either from just one or multiple pages. Adapted from [@minimaxir](https://github.com/minimaxir/facebook-page-post-scraper)'s scripts.
* [get_fb_comments_from_fb.py](#): This script allows you to get the comments from each Facebook group or page. Adapted from [@minimaxir](https://github.com/minimaxir/facebook-page-post-scraper)'s scripts.


## How to run each script
1. Run your script with the bash command `python foldername/scriptname.py` to generate a csv of tweets or Facebook posts. Then, go make do some journalism-ing!
